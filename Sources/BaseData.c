...
void initGame()
{
	...
	sprintf(character[_PLAYER_],       "★");
	sprintf(character[_WALL_],         "■");
	sprintf(character[_BALL_],         "◎");
	sprintf(character[_EMPTY_BOX_],    "()");
	sprintf(character[_FILLED_BOX_],   "●");
	sprintf(character[_OUT_OF_MAP_],   "..");
...
void getLeaderboard(char* dst, const int stage)
{
	FILE* fp;
	char path[1024], fname[16];
	
	_getcwd(path, 1024);		// path of root of this project directory
	sprintf(fname, "Stage%02d.dat", stage);
	sprintf(path, "%s\\Resources\\Ranking\\%s", path, fname);
	fp = fopen(path, "r+");
	if (fp == NULL)
	{
		fp = fopen(path, "w+");
